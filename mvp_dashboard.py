...
from core.data_loader import load_ohlcv, get_symbol
...
from components.monthly_activity_panel import render_monthly_activity
...
# =====================================================
    ...
    st.caption(f"Price source: Yahoo Finance / local cache · {get_symbol(cfg.market)} · {cfg.timeframe}. Cache freshness and missing sessions are not independently verified.")
    if cfg.market == "NAS100":
        st.info("NAS100 currently uses the Nasdaq-100 cash index (^NDX) as a research proxy. It is not a broker CFD or NQ/MNQ futures execution feed.")
    ...
    if validation:
        ...
        render_monthly_activity(validation)
    st.caption("Execution: signal on completed bar, entry at next available open; protective exits active immediately. Costs are deducted as round-trip cash charges. Drawdown uses bar-close equity, not worst intrabar equity. Sizing uses fractional units without broker lot or margin constraints.")
    ...
    st.markdown('<div class="va-section-title">Closed-trade cumulative P&L</div>', unsafe_allow_html=True)
    st.caption("This chart shows realized trade P&L. Reported drawdown also includes unrealized losses at bar closes.")
    ...
    st.caption("signal_time identifies the source candle; the decision uses its completed close. entry_time and exit_time identify execution candles, not exact intrabar timestamps.")
    if trades_df.empty:
